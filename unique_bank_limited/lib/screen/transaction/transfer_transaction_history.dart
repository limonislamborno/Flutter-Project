...
class TransferTransactionHistoryPage extends StatefulWidget {
  final String fromAccountNumber;

  const TransferTransactionHistoryPage({Key? key, required this.fromAccountNumber}) : super(key: key);

  @override
  _TransferTransactionHistoryPageState createState() => _TransferTransactionHistoryPageState();
}

class _TransferTransactionHistoryPageState extends State<TransferTransactionHistoryPage> {
  ...
  Future<void> _fetchTransactionHistory() async {
    try {
      ...
      final response = await http.get(Uri.parse('http://localhost:8086/api/transfer/transaction-history?fromAccountNumber=${widget.fromAccountNumber}&startDate=$_startDate&endDate=$_endDate'));
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('Transfer Transaction History'),
      ),
      body: Column(
        children: [
          Container(
