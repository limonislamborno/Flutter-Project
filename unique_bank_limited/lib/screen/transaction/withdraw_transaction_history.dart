...
class WithdrawTransactionHistoryPage extends StatefulWidget {
  final String accountNumber;

  const WithdrawTransactionHistoryPage({Key? key, required this.accountNumber}) : super(key: key);

  @override
  _WithdrawTransactionHistoryPageState createState() => _WithdrawTransactionHistoryPageState();
}

class _WithdrawTransactionHistoryPageState extends State<WithdrawTransactionHistoryPage> {
  ...
  Future<void> _fetchTransactionHistory() async {
    try {
      ...
      final response = await http.get(Uri.parse('http://localhost:8086/api/withdraw/transaction-history?accountNumber=${widget.accountNumber}&startDate=$_startDate&endDate=$_endDate'));
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('Withdraw Transaction History'),
      ),
      body: Column(
        children: [
          Container(
